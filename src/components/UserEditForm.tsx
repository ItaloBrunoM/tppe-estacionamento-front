...
interface UserFormProps {
  ...
  export function UserEditForm({
      ...
      if (user) {
        if (user.pessoa) {
            ...
            if (formattedCpfLoaded.length > 3) {
              formattedCpfLoaded = formattedCpfLoaded.replace(/(\d{3})(\d)/, '$1.$2');
            }
            if (formattedCpfLoaded.length > 7) {
              formattedCpfLoaded = formattedCpfLoaded.replace(/(\d{3})\.(\d{3})(\d)/, '$1.$2.$3');
            }
            if (formattedCpfLoaded.length > 11) {
              formattedCpfLoaded = formattedCpfLoaded.replace(/(\d{3})\.(\d{3})\.(\d{3})(\d{1,2})/, '$1.$2.$3-$4');
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        if (user) {
          await api.put(`/usuarios/${user.id}`, { user_data: userData, pessoa_data: pessoaData });
        } else {
            ...
            <form onSubmit={handleSubmit}>
              <fieldset>
                ...
                <input
                  ...
                  inputMode="numeric"
                />
                {cpfError && <span className="error-message cpf-error">{cpfError}</span>}
